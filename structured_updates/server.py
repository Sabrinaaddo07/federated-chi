...
class BaselineFedAvg(Strategy):
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        per_round = self.model_bytes * len(results) * (1 + self.update_rate)
        self.cumulative_overhead_bytes += per_round

        print(f"  Round {server_round:2d} — Global test accuracy: {global_acc:.4f}  "
              f"| {len(results)} clients | rate={self.update_rate:.2f}")

        if self.csv_writer is None:
            self.csv_file = open("results_structuredup.csv", "w", newline="")
...
def main():
    ...
    parser.add_argument("--update_rate", type=float, default=1.0,
                        help="Fraction of params kept per update (0 to 1)")
    args = parser.parse_args()

    print("=" * 55)
    print("  STRUCTURED UPDATES — Federated Learning")
    print(f"  {args.num_clients} clients  |  {args.num_rounds} rounds")
    print(f"  Update rate: {args.update_rate:.2f}  |  top-k sparsification")
