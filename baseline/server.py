...
class BaselineFedAvg(Strategy):
    ...
    def configure_fit(self, server_round, parameters, client_manager):
        ...
        while num_available < self.num_clients:
            time.sleep(2)
            num_available = client_manager.num_available()
            print(f"  Waiting for clients... {num_available}/{self.num_clients}")
        clients = client_manager.sample(
            num_clients=num_available, min_num_clients=self.num_clients
        )
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        if self.csv_writer is None:
            self.csv_file = open("results_baseline.csv", "w", newline="")
            self.csv_writer = csv.writer(self.csv_file)
            self.csv_writer.writerow([
                "round", "global_accuracy", "num_participated", "participating_cids"
            ])
        self.csv_writer.writerow([
            server_round, f"{global_acc:.4f}",
            len(results), sorted(participating_cids),
        ])
        self.csv_file.flush()
...
def main():
    ...
    print("=" * 55)
    print("  BASELINE FEDERATED LEARNING EXPERIMENT")
