...
from common import (
    ...
    load_client_data_iid, load_client_data_noniid, NUM_CLASSES,
)
...
class DistributionClient(fl.client.NumPyClient):
    def __init__(self, cid, num_clients, distribution):
        self.cid = cid
    ...
    def _load_data(self):
        if self.distribution == "noniid":
            self.X_train, self.X_test, self.y_train, self.y_test = \
                load_client_data_noniid(self.cid, self.num_clients)
        else:
            self.X_train, self.X_test, self.y_train, self.y_test = \
                load_client_data_iid(self.cid, self.num_clients)
        self.model = create_model()
        self.all_classes = NUM_CLASSES
        self.data_loaded = True
        print(f"  Client {self.cid} ({self.distribution}) — "
              f"{len(self.X_train)} train, {len(self.X_test)} test samples")
    ...
    def fit(self, parameters, config):
        ...
        return new_weights, len(self.X_train), {"cid": self.cid}
...
def main():
    ...
    print(f"--- Client {args.cid} starting ({args.distribution}) ---")
    print(f"  Connecting to server at {args.server_address}")
    fl.client.start_numpy_client(
        server_address=args.server_address,
        client=DistributionClient(
            args.cid, args.num_clients, args.distribution
        ),
    )
