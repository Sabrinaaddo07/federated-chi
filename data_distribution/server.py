...
class BaselineFedAvg(Strategy):
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        tag = self.distribution.upper()
        print(f"  Round {server_round:2d} — [{tag}] Global test accuracy: {global_acc:.4f}  "
              f"| {len(results)} clients")

        if self.csv_writer is None:
            self.csv_file = open("results_distribution.csv", "w", newline="")
...
def main():
    ...
    print("=" * 55)
    print("  DATA DISTRIBUTION — Federated Learning")
