...
def create_model():
    return SGDClassifier(loss="log_loss", warm_start=True, random_state=42)
...
def load_server_test_data():
    _, X_test, _, y_test = train_test_split(
        *load_digits(return_X_y=True), test_size=0.2, random_state=42
    )
    X_test = X_test.astype(np.float64) / 16.0
